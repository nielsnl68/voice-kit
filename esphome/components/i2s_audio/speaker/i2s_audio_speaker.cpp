...
void I2SAudioSpeaker::setup() {
  ...
  this->buffer_queue_ = xStreamBufferCreate(BUFFER_COUNT * BUFFER_SIZE, this->sample_byte_size());
...
void I2SAudioSpeaker::player_task(void *params) {
  ...
  while (true) {
    if (this_speaker->buffer_queue_ != nullptr) {
      int ret = xStreamBufferReceive(this_speaker->buffer_queue_, &sample, this_speaker->sample_byte_size(),
                                     10 / portTICK_PERIOD_MS);
      if (ret == this_speaker->sample_byte_size()) {
        if (!is_playing) {
        ...
        esp_err_t err = i2s_write(this_speaker->parent_->get_port(), &sample, this_speaker->sample_byte_size(),
        ...
        if (err != ESP_OK) {
          event.err = err;
          xQueueSend(this_speaker->event_queue_, &event, 10 / portTICK_PERIOD_MS);
        } else if (bytes_written != this_speaker->sample_byte_size()) {
          event.err = ESP_FAIL;
          xQueueSend(this_speaker->event_queue_, &event, 10 / portTICK_PERIOD_MS);
        } else {
          error_count = 0;
...
void I2SAudioSpeaker::handle_stop() {
      ...
      if (xQueueReceive(this->event_queue_, &event, 0) == pdTRUE) {
        switch (event.type) {
          ...
          case TaskEventType::WARNING:
            if (event.data < 0) {
              ESP_LOGW(TAG, "Read data size mismatch: %d instead of %d", -event.data, this->sample_byte_size());

            } else if (event.data > 0) {
              ESP_LOGW(TAG, "Write data size mismatch: %d instead of %d", event.data, this->sample_byte_size());
...
bool I2SAudioSpeaker::has_buffered_data() const {
  if (this->is_failed()) {
    ESP_LOGE(TAG, "Cannot check for buffered data, speaker failed to setup");
    return false;
  }
...
size_t I2SAudioSpeaker::available_space() const {
  if (this->is_failed()) {
    ESP_LOGE(TAG, "Cannot get available space, speaker failed to setup");
    return 0;
  }
...
void I2SAudioSpeaker::set_state_(speaker::State state) {
  ...
  ESP_LOGV(TAG, "State changed from %s to %s", LOG_STR_ARG(state_to_string(this->state_)),
